...
function Nav() {
    ...
    return (
        <nav className="relative flex items-center justify-between">
            ...
            <div>
                <Link to={'/'} className="relative block self-center">
                    <img src="./favicon.png" alt="logo" width={50} height={50} />
                </Link>
            </div>
            <ul ref={navLink} className="flex gap-8 bg-white max-sm:p-4 max-sm:flex-col max-sm:gap-4 max-sm:shadow max-sm:rounded max-sm:absolute max-sm:z-10 max-sm:right-0 max-sm:top-12 max-sm:hidden">
                <li>
                    <Link to={'/'} className="relative block self-center">Product</Link>
                </li>
                <li>
                    Recomended
                </li>
            ...
            <div className="flex gap-8">
                ...
                <Link to={'/cart'} className="relative block self-center">
                    {cart.length >= 1 && <span className="absolute flex justify-center items-center w-4 h-4 -top-2 -right-2 bg-red-500 rounded-full text-xs text-white">{cart.length}</span>}
