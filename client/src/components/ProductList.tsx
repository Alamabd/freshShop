...
function ProductList() {
    ...
    return (
        <div className="mt-12">
            ...
            {/* Product list */}
            <div className="py-2 px-1 grid grid-cols-4 gap-4 max-lg:grid-cols-3 max-sm:grid-cols-2">
                {
                    product ? 
                        product.map((product) => {
                            return(
                                <div key={product._id} className="p-4 relative flex flex-col gap-2 rounded-xl shadow">
                                    <img src={product.img} alt="apple" className="w-full aspect-square rounded-xl" />
                                    <div>
                                        <h2 className="font-semibold text-nowrap">{product.name}</h2>
                                        <p className="text-sm text-nowrap">{FormatIdr(parseInt(product.price))}</p>
                                        <div className="flex items-center justify-between">
                                            <p className="text-lg text-yellow-400 max-sm:text-base">{Rating(product.rating)}</p>
                                            <button onClick={() => adToCart(product)} className="mt-2 p-2 float-end text-nowrap bg-emerald-500 text-white text-xs rounded shadow max-sm:text-xs">
